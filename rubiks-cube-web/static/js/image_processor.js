    ...
    async processImage(imageElement) {
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                // 1. Find the largest square contour (the cube face).
                const contour = this.findFaceContour(src);
                if (!contour) {
                    return reject("No cube face found in the image.");
                }

                // 2. Apply perspective transform to get a flat, square image of the face.
                const warped = this.getPerspectiveWarp(src, contour);

                // 3. Sample the 9 sticker colors from the warped image.
                const colors = this.sampleStickerColors(warped);

...
class CubeSolverJS {
    ...
    solveWhiteCross() {
        ...
        // Move all white edges to the yellow face (bottom)
        const edges = [
            ['up', 7], ['up', 5], ['up', 1], ['up', 3], // Top layer
            ['front', 3], ['front', 5], // Middle layer front
            ['right', 3], ['right', 5], // Middle layer right
            ['back', 3], ['back', 5],   // Middle layer back
            ['left', 3], ['left', 5]    // Middle layer left
        ];

        for (let i = 0; i < 8; i++) { // Loop a few times to handle complex cases
            for (const [face, idx] of edges) {
                if (this.cube.state[face][idx] === white) {
                    // This is a simple but brute-force way to move edges down
                    this.execute("R U R' F'");
                }
            }
        }

        // Now align edges on the bottom and move them up
        for (const color of ['R', 'B', 'O', 'G']) {
            const targetFace = centers[color];
            
            // Find the edge with White and the current color on the bottom face
            for (let i = 0; i < 4; i++) {
                if (this.cube.state.down[1] === white && this.cube.state.front[7] === color) {
                    break;
                }
                this.execute('D');
            }
            // Move the aligned edge up to the white face
            this.execute('F2');
            this.execute('y'); // Rotate to the next face
        }
    }
        ...
        while (changed) {
            ...
            while (i < simplified.length) {
                if (i + 1 < simplified.length) {
                    ...
                    // U U -> U2
                    if (m1 === m2) {
                        newMoves.push(m1[0] + '2');
                        i += 2;
                        changed = true;
                        continue;
                    }
                    // U U' -> cancel
                    if (m1[0] === m2[0] && m1.length !== m2.length) {
                        i += 2;
                        changed = true;
                        continue;
                    }
                     // U2 U -> U'
                    if (m1[0] === m2[0] && m1.endsWith('2')) {
                        newMoves.push(m2.includes("'") ? m1[0] : m1[0] + "'");
                        i += 2;
                        changed = true;
                        continue;
                    }

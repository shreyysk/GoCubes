...
class CubeModel:
    ...
    def from_web_format(self, web_state: Dict[str, List[str]]):
        """
        Sets the cube's state from the web format dictionary.
        
        Args:
            web_state: Dictionary with keys 'up', 'down', 'front', etc.
                       and values being lists of 9 color characters (W, Y, R, etc.).
        """
        face_order = ['up', 'right', 'front', 'down', 'left', 'back']
        if not all(key in web_state for key in face_order):
            raise ValueError("Web format state is missing one or more faces.")

        # Validate each face has 9 stickers
        for face in face_order:
            if len(web_state[face]) != 9:
                raise ValueError(f"Face {face} must have exactly 9 stickers, got {len(web_state[face])}")

        # The color of the center sticker determines the face's identity
        color_to_face_map = {}
        center_colors = set()
        
        for face in face_order:
            center_color = web_state[face][4]
            if center_color in center_colors:
                raise ValueError(f"Duplicate center color detected: {center_color}")
            center_colors.add(center_color)
            
            # Map color to face notation
            face_notation = {
                'up': 'U', 'right': 'R', 'front': 'F',
                'down': 'D', 'left': 'L', 'back': 'B'
            }
            color_to_face_map[center_color] = face_notation[face]

        if len(color_to_face_map) != 6:
            raise ValueError("Invalid state: Must have exactly 6 unique center colors")

        # Flatten the web state into a single list
        flat_state_colors = []
        for face in face_order:
            flat_state_colors.extend(web_state[face])
        
        # Translate the color characters to face initials
        try:
            self.state = [color_to_face_map[c] for c in flat_state_colors]
        except KeyError as e:
            raise ValueError(f"Invalid sticker color '{e.args[0]}' found which does not match any center color.")
    ...
    def _rotate_face(self, face: str):
        ...
        # Apply cycles as tuples of (from, to) indices
        for from_idx, to_idx in cycles:
    ...
    def validate(self) -> Tuple[bool, Optional[str]]:
        ...
        # Advanced solvability check (Parity etc.)
        is_solvable, error = self._check_solvability()
        if not is_solvable:
            return False, error
    ...
    def _check_solvability(self) -> Tuple[bool, Optional[str]]:
        """
        Simplified solvability check - just verify basic constraints
        """
        # Check that we have exactly 9 of each color
        color_count = {}
        for sticker in self.state:
            color_count[sticker] = color_count.get(sticker, 0) + 1
        
        for color in 'URFDLB':
            if color_count.get(color, 0) != 9:
                return False, f"Invalid color count for {color}"
        
        # Check that center pieces are all different
        centers = [self.state[4], self.state[13], self.state[22], 
                   self.state[31], self.state[40], self.state[49]]
        if len(set(centers)) != 6:
            return False, "Duplicate center colors"
